...
class Cfg:
    ...
    # output
    out_dir: str = "/home/nng/koopman_project/out_align_koopman_2"
    seed: int = 0

    # -------- Stage 1: source pretrain --------
    total_steps_src: int = 200000
    batch_size: int = 256
    lr: float = 3e-4
    hidden_dim: int = 256
    n_layers: int = 3
    dyn_cons_update_freq: int = 2
    src_dyn_weight: float = 0.1   # 关键：降低 dyn-cons 对 source encoder 的牵引
    ...
    # fixed: 用下面给定常数
    # max_abs: 用绝对值最大值
    # robust_q995: 用 |a| 的 99.5% 分位数（推荐）
    action_scale_mode: str = "robust_q995"
    franka_action_scale: float = 1.5   # fixed 时使用
    ur_action_scale: float = 1.0       # fixed 时使用
...
class SrcAgent(nn.Module):
    def __init__(
        ...
        # 关键修改：
        # 1) obs_enc 不再 tanh，避免 source latent 饱和/塌缩
        # 2) fwd_dyn 输出也不再 tanh，因为它预测的是 latent state
...
class ObsActAligner:
    def __init__(
        ...
        self.z_t2s = build_mlp(src.lat_obs_dim, src.lat_obs_dim, n_layers, hidden_dim, activation="relu", out_act="identity").to(device)
        self.z_s2t = build_mlp(src.lat_obs_dim, src.lat_obs_dim, n_layers, hidden_dim, activation="relu", out_act="identity").to(device)
        self.u_t2s = build_mlp(src.lat_act_dim, src.lat_act_dim, n_layers, hidden_dim, activation="relu", out_act="identity").to(device)
        self.u_s2t = build_mlp(src.lat_act_dim, src.lat_act_dim, n_layers, hidden_dim, activation="relu", out_act="identity").to(device)
    ...
    def update_disc(self, src_s, src_a, tgt_s, tgt_a):
        ...
        with torch.no_grad():
            ...
            # target latent -> mapped to source latent space
            z_t = self.tgt.obs_enc(tgt_s)
            u_t = self.tgt.act_enc(torch.cat([tgt_s, tgt_a], dim=-1))
            fake_z = self.z_t2s(z_t)
            fake_u = self.u_t2s(u_t)

            # normalize before feeding discriminator
            real_z_n = self.norm_z(real_z)
            fake_z_n = self.norm_z(fake_z)
            real_u_n = self.norm_u(real_u)
            fake_u_n = self.norm_u(fake_u)
        ...
        loss_act = self.lat_act_disc(fake_u_n).mean() - self.lat_act_disc(real_u_n).mean()
        gp_act = self._grad_penalty(self.lat_act_disc, real_u_n, fake_u_n)
    ...
    def update_gen(self, src_s, src_a, src_sn, tgt_s, tgt_a, tgt_sn):
        ...
        # map target -> source latent space
        z_s_hat = self.z_t2s(z_t)
        u_s_hat = self.u_t2s(u_t)
        z_s_next_hat = self.z_t2s(z_t_next)

        # adversarial losses: discriminate in source latent space
        adv_z = -self.lat_obs_disc(z_s_hat).mean()
        adv_u = -self.lat_act_disc(u_s_hat).mean()

        # cycle consistency
        z_t_cyc = self.z_s2t(z_s_hat)
        ...
        loss = (adv_z + adv_u) + self.lmbd_cyc * cycle_loss + self.lmbd_dyn * dyn_loss + loss_ae_a
        loss.backward()
...
def load_stage2(tgt: TgtAgent, aligner: ObsActAligner, tgt_ckpt: str, bundle_ckpt: str, device):
    if os.path.exists(tgt_ckpt):
        tgt.load_state_dict(torch.load(tgt_ckpt, map_location=device))
    if not os.path.exists(bundle_ckpt):
        raise FileNotFoundError(f"Stage2 bundle not found: {bundle_ckpt}")
    bundle = torch.load(bundle_ckpt, map_location=device)
...
def main():
    ...
    print(
        f"[INFO] Franka flat: N={fr_s2.shape[0]}, obs_dim={fr_s2.shape[1]}, act_dim={fr_a2.shape[1]}, "
        f"action_scale={fr_scale:.6f}"
    )
    print(
        f"[INFO] UR flat:     N={ur_s2.shape[0]}, obs_dim={ur_s2.shape[1]}, act_dim={ur_a2.shape[1]}, "
        f"action_scale={ur_scale:.6f}"
    )
    ...
    tgt_ckpt = cfg.load_stage2_ckpt if cfg.load_stage2_ckpt else os.path.join(tgt_dir, "tgt_agent.pt")
    bundle_ckpt = os.path.join(tgt_dir, "stage2_bundle.pt")
    ...
    if cfg.skip_stage1:
        ...
        src_opt = torch.optim.Adam(src.parameters(), lr=cfg.lr)
        t0 = time.time()
        ...
        for step in range(1, cfg.total_steps_src + 1):
            ...
            if step % 2000 == 0:
                ...
                print(
                    ...
                    f"inv={dyn_logs['inv']:.4f} fwd={dyn_logs['fwd']:.4f} "
                    f"dt={dt:.1f}s"
                )
                t0 = time.time()
    ...
    with torch.no_grad():
        ...
        src_u_sample = src.act_enc(torch.cat([fr_buf.s[:n_stat], fr_buf.a[:n_stat]], dim=-1))

        z_mean = src_z_sample.mean(dim=0, keepdim=True)
        z_std = src_z_sample.std(dim=0, keepdim=True).clamp_min(1e-6)

        u_mean = src_u_sample.mean(dim=0, keepdim=True)
        u_std = src_u_sample.std(dim=0, keepdim=True).clamp_min(1e-6)
    # ---------- Stage 2 ----------
    tgt = TgtAgent(
    ...
    if cfg.skip_stage2:
        print(f"[STAGE 2] skip alignment, loading tgt ckpt: {tgt_ckpt}")
        load_stage2(tgt, aligner, tgt_ckpt, bundle_ckpt, device)
    else:
        ...
        for step in range(1, cfg.total_steps_align + 1):
            ...
            if step % 2000 == 0:
                ...
                print(
                    ...
                    f"inv={glogs['inv']:.3f} fwd={glogs['fwd']:.3f} "
                    f"dt={dt:.1f}s"
                )
                t0 = time.time()
