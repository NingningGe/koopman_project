...
my_dofbot = my_world.scene.get_object(task_params["robot_name"]["value"])
my_dofbot.disable_gravity()
...
data = []
segment_data = {}  
initial_angles = [0.0, 0.0, -np.pi/6,  np.pi / 6, 0.0, 0,0,0,0,0,0]  
noise_angles = np.array(initial_angles) + np.concatenate([np.random.uniform(-0.3,0.3,5),[0,0,0,0,0,0]])
my_dofbot.set_joint_positions(noise_angles)
initial_velocities = np.zeros(11)
noise_velocities = initial_velocities + np.concatenate([np.random.uniform(-0.3,0.3,5),[0,0,0,0,0,0]])
my_dofbot.set_joint_velocities(noise_velocities)

segment_counter = 0  

while (time.time() - start_time2) < 1200:
    A1 = np.random.uniform(0,0.003)
    A2 = np.random.uniform(0,0.002) 
    A3 = np.random.uniform(0,0.0005)
    A4 = np.random.uniform(0,0.0005)
    A5 = np.random.uniform(0,0.0001)
    T1,T2,T3,T4,T5 = np.random.uniform(0.1,1,5)
    theta1, theta2, theta3, theta4, theta5 = np.random.uniform(0,2 * np.pi,5)
    if (time.time() - start_time) <= 1:
        ...
        noise_std = [0.0006,0.0004,0.0001,0.0001,0.00002,0,0,0,0,0,0]
        noisy_torques = [
        ...
        positions = my_dofbot.get_joint_positions()
        velocities = my_dofbot.get_joint_velocities()
...
np.save('/home/cxf/franka/traj_origin_file/all_segment_data_dofbot2.npy', segment_data)
# np.save('/home/cxf/桌面/210310205/data/dotfot_1200.npy', segment_data)
