...
noise_velocities = np.concatenate([np.random.uniform(-0.3,0.3,6)])
my_ur.set_joint_velocities(noise_velocities)

segment_counter = 0  

while (time.time() - start_time2) < 350:
    A1,A2,A3,A4,A5,A6 = np.random.uniform(0,0.3,6)
    ...
    if (time.time() - start_time) <=1:
        ...
        noise_std = [0.06, 0.06,0.06,0.06,0.06,0.06]
        noisy_speeds = [
...
np.save('/home/cxf/franka/traj_speed_file/all_segment_testdata2_ur.npy', segment_data)
