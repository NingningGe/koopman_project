...
def Klinear_loss(X1,net,dnet,enc_net1,enc_net4,dec_net1,dec_net4,mse_loss,gamma,Nstate,primary_udim1):
...
def train(env_name, train_steps=200000, suffix="", all_loss=0, \
    ...
    for i in range(train_steps):
        ...
        Kloss,Y_loss, x_loss, x_enc_loss, u_rec_loss, U_rec_loss ,x_enc_dec_loss, Y_enc_loss,dec_Y_loss = Klinear_loss(X1,net,dnet,enc_net1,enc_net4,dec_net1,dec_net4,mse_loss,gamma,Nstate,primary_udim1)
        Eloss = Eig_loss(net)
        ...
        if (i + 1) % eval_step == 0:
            # K loss
            with torch.no_grad():
                Kloss,Y_loss, x_loss, x_enc_loss, u_rec_loss, U_rec_loss ,x_enc_dec_loss, Y_enc_loss ,dec_Y_loss= Klinear_loss(Ktest_data1,net,dnet,enc_net1,enc_net4,dec_net1,dec_net4,mse_loss,gamma,Nstate,primary_udim1)
                Eloss = Eig_loss(net)
                loss = Kloss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--suffix", type=str, default="franka")
